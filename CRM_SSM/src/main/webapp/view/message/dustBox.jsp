...
<body>
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-12"><h2>垃圾箱</h2></div>
        </div>
        <!-- 搜索框 -->
		<div class="row" style="margin-bottom: 10px;">
			<form class="form-inline col-md-12" autocomplete="off" action="message">
				<input type="hidden" name="method" value="dustBox">
                    ...
                    <tbody>
                    	<c:forEach items="${pageBean.content}" var="message">
							<tr>
								...
								<td>
									<a class="btn btn-link" href ="message?method=selectById&message_id=${message.message_id}"
									 data-toggle="tooltip" data-placement="left" title="查看邮件">
										<i class="fa fa-paper-plane" aria-hidden="true" style="color: black;"></i>
									</a>
									<a class="btn btn-link" href ="message?method=repeal&message_id=${message.message_id}&sender=${message.sender}"
									 data-toggle="tooltip" data-placement="top" title="撤销删除">
										<i class="fa fa-undo" aria-hidden="true" style="color: black;"></i>
									</a>
									<a class="btn btn-link" href ="message?method=dustDeleteShift&message_id=${message.message_id}
			...
			<div class="col-md-6" style="text-align: right;">
				...
				<ul class="pagination">
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a href="message?method=dustBox&pageNo=${pageBean.pageNo-1}&keyword=${keyword}&colName=${colName}">&laquo;</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 补全页码 -->
					<c:forEach begin="1" end="${pageBean.totalPage}" var="i">
						...
						<c:choose>
							<c:when test="${i == pageBean.pageNo}">
								<li class="active"><a href="message?method=dustBox&pageNo=${i}&keyword=${keyword}&colName=${colName}">${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="message?method=dustBox&pageNo=${i}&keyword=${keyword}&colName=${colName}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 处理下一页的操作 -->
					<c:choose>
						...
						<c:otherwise>
							<li>
							<a href="message?method=dustBox&pageNo=${pageBean.pageNo+1}&keyword=${keyword}&colName=${colName}&sender=${USER.user_id}">&raquo;</a></li>
						</c:otherwise>
					</c:choose>
				</ul>
				</nav>
