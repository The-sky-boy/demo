...
<body>
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-12"><h2>发件箱</h2></div>
        </div>
        <!-- 搜索框 -->
                    ...
                    <tbody>
                    	<c:forEach items="${pageBean.content}" var="message">
							<tr>
								...
								<td><p data-toggle="tooltip" title="${message.content}" data-placement="right">${message.content}</p></td>
								<td>${message.send_time}</td>
								<td>
									<a class="btn btn-link" href ="message?method=selectById&message_id=${message.message_id}&pageNo=${pageBean.pageNo}&keyword=${keyword}&colName=${colName}"
									 data-toggle="tooltip" data-placement="left" title="查看邮件">
										<i class="fa fa-paper-plane" aria-hidden="true" style="color: black;"></i>
									</a>
									<a class="btn btn-link" href ="message?method=senderDeleteToDustBox&message_id=${message.message_id}&pageNo=${pageBean.pageNo}&keyword=${keyword}&colName=${colName}"
									 data-toggle="tooltip" data-placement="top" title="删除">
										<i class="fa fa-trash" style="color: black;"
											aria-hidden="true"></i>
									</a>
									<a class="btn btn-link" href ="message?method=senderDeleteShift&message_id=${message.message_id}&pageNo=${pageBean.pageNo}&keyword=${keyword}&colName=${colName}"
			...
			<div class="col-md-6" style="text-align: right;">
				...
				<ul class="pagination">
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a href="message?method=sendBox&pageNo=${pageBean.pageNo-1}&keyword=${keyword}&colName=${colName}">&laquo;</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 补全页码 -->
					<c:forEach begin="1" end="${pageBean.totalPage}" var="i">
						...
						<c:choose>
							<c:when test="${i == pageBean.pageNo}">
								<li class="active"><a href="message?method=sendBox&pageNo=${i}&keyword=${keyword}&colName=${colName}">${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="message?method=sendBox&pageNo=${i}&keyword=${keyword}&colName=${colName}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 处理下一页的操作 -->
					<c:choose>
						...
						<c:otherwise>
							<li>
							<a href="message?method=sendBox&pageNo=${pageBean.pageNo+1}&keyword=${keyword}&colName=${colName}&sender=${USER.user_id}">&raquo;</a></li>
						</c:otherwise>
					</c:choose>
				</ul>
				</nav>
