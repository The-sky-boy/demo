...
<body>
    <div class="container-fluid">
		...
		<div class="row" style="margin-bottom: 10px;">
			<form class="form-inline col-md-6" autocomplete="off" action="dept">
			...
			<div class="col-md-6">
				<a class="btn btn-primary pull-right" href="view/department/addDept.jsp?pageNo=${pageBean.pageNo}&keyword=${keyword}">添加部门</a>
			</div>
		</div>
        <!-- 数据表格 -->
        <div class="row">
                    ...
                    <tbody>
                    	<c:forEach items="${pageBean.content}" var="dept">
							<tr>
								...
								<td>
									<a class="btn btn-link" href="dept?method=selectById&department_id=${dept.department_id}&pageNo=${pageBean.pageNo}&keyword=${requestScope.keyword}">
										<span class="glyphicon glyphicon-pencil" style="color: black;" aria-hidden="true"></span>
									</a>
									<a class="btn btn-link" href ="dept?method=delete&department_id=${dept.department_id}&pageNo=${pageBean.pageNo}&keyword=${requestScope.keyword}">
			...
			<div class="col-md-6" style="text-align: right;">
				...
				<ul class="pagination">
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a href="dept?method=show&pageNo=${pageBean.pageNo-1}&keyword=${keyword}">&laquo;</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 补全页码 -->
					<c:forEach begin="1" end="${pageBean.totalPage}" var="i">
						...
						<c:choose>
							<c:when test="${i == pageBean.pageNo}">
								<li class="active"><a href="dept?method=show&pageNo=${i}&keyword=${keyword}">${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="dept?method=show&pageNo=${i}&keyword=${keyword}">${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 处理下一页的操作 -->
					<c:choose>
						...
						<c:otherwise>
							<li>
							<a href="dept?method=show&pageNo=${pageBean.pageNo+1}&keyword=${keyword}">&raquo;</a></li>
						</c:otherwise>
					</c:choose>
				</ul>
				</nav>
